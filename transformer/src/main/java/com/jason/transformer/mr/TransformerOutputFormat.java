package com.jason.transformer.mr;
...
import com.jason.transformer.service.impl.DimensionConverterImpl;
...
public class TransformerOutputFormat extends OutputFormat<BaseDimension, BaseStatsValueWritable> {
    ...
    public class TransformerRecordWriter extends RecordWriter<BaseDimension, BaseStatsValueWritable> {
        ...
        private IDimensionConverter converter = null; //专门操作dimension表的对象，具体实现类DimensionConverterImpl
        ...
        //具体执行写操作，输出的key顶级父类BaseDimension，输出value的顶级父类BaseStatsValueWritable
        @Override
        public void write(BaseDimension key, BaseStatsValueWritable value) throws IOException, InterruptedException {
            ...
            try {
                ...
                int count = 1;
                //将后续要执行的操作进行分batch，每个batch中存储一定的sql操作，通过kpi到config中查找，如果没有的sql获取，有的直接从map中获取
                if (map.get(kpi) == null) {
                ...
                IOutputCollector collector = (IOutputCollector) clazz.newInstance(); //具体实现StatsUserNewInstallUserCollector
