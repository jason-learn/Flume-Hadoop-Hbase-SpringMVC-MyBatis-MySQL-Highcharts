...
-- 5. 上传transformer-0.0.1.jar到hdfs的/jason/transformer文件夹中
-- 6. 创建hive的function
create function platform_convert as 'com.jason.transformer.hive.PlatformDimensionUDF' using jar 'hdfs://hh:8020/jason/transformer/transformer-0.0.1.jar';  
create function date_convert as 'com.jason.transformer.hive.DateDimensionUDF' using jar 'hdfs://hh:8020/jason/transformer/transformer-0.0.1.jar';  
...
-- 9. sqoop脚步编写(统计会话角度)
sqoop export --connect jdbc:mysql://localhost:3306/report --username hive --password hive --table stats_view_depth --export-dir /hive/bigdater.db/stats_view_depth/* --input-fields-terminated-by "\\01" --update-mode allowinsert --update-key platform_dimension_id,data_dimension_id,kpi_dimension_id
